...
namespace WindowsFormsApp1
{
    public partial class MainForm : Form
    {
        ...
        {
            InitializeComponent();
            this.FormBorderStyle = FormBorderStyle.Sizable; // size form 
                ...
                case "Clients":
                    ...
                    if (ClientToDelete != null)
                    {
                        try
                        {
                            ...
                            if (result == DialogResult.Yes)
                            {
                                var match = Context.TraineesClassesEnrollments.Where(tranieid => tranieid.TraineesID == ClientToDelete.ID).ToList();
                                foreach(var tranie in match)
                                {
                                    Context.TraineesClassesEnrollments.Remove(tranie);
                                    Context.SaveChanges();
                                }
                                //Context.SaveChanges();
                                Context.Trainees.Remove(ClientToDelete);
                                Context.SaveChanges();
                            }
                            else
                            { BTN_Classes_Click(sender, e); }
        ...
        private void SearchButton_Click(object sender, EventArgs e)
        {
            ...
            switch (currentPage)
            {
                ...
                default:
                    // Handle the case where the entity type is not found
                    MessageBox.Show("Entity type not found");
                    return;
            }
